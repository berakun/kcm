...
exports.getPos = async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query(
      'SELECT p.*, u.name as creator_name FROM purchase_orders p LEFT JOIN users u ON p.created_by = u.id ORDER BY p.id DESC'
    );
...
exports.createPo = async (req, res) => {
  const {
    ...
    ordered_by, purchasing, prepared_by, approved_by, term_of_payment
  } = req.body;
    ...
    await db.query(
      `INSERT INTO purchase_orders
        (po_number, to_supplier, date, phone, attn, rab_id, items, projects, material_ref,
         ...
         ordered_by, purchasing, prepared_by, approved_by, term_of_payment, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
